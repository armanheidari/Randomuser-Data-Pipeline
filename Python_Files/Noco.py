...
sys.path.append(str(root_path / 'Python_Files'))
...
class Nocodb:
    ...
    def load_token_from_file(self) -> str:
        try:
            with open(root_path / 'Administrator_Scripts/Nocodb/token.txt', 'r', encoding='utf-8') as f:
                token = f.read().strip()
    ...
    def save_token(self, token: str):
        with open(root_path / 'Administrator_Scripts/Nocodb/token.txt', 'w', encoding='utf-8') as f:
            f.write(token)

    def save_info(self, email: str, password: str):
        with open(root_path / 'Administrator_Scripts/Nocodb/email.txt', 'w', encoding='utf-8') as f:
            f.write(email)

        with open(root_path / 'Administrator_Scripts/Nocodb/password.txt', 'w', encoding='utf-8') as f:
            f.write(password)
    
    def load_info(self) -> dict:
        info = {}
        try:
            with open(root_path / 'Administrator_Scripts/Nocodb/email.txt', 'r', encoding='utf-8') as f:
                info['email'] = f.read().strip()

            with open(root_path / 'Administrator_Scripts/Nocodb/password.txt', 'r', encoding='utf-8') as f:
    ...
    def load_base_id(self):
        try:
            with open(root_path / 'Administrator_Scripts/Nocodb/base_id.txt', 'r', encoding='utf-8') as f:
    ...
    def save_base_id(self, base_id: str):
        with open(root_path / 'Administrator_Scripts/Nocodb/base_id.txt', 'w', encoding='utf-8') as f:
            f.write(base_id)
    ...
    def create_table(self, title: str, columns: dict):
        ...
        if response.status_code == 200:
            ...
            # self._table_id = response.json()['id']
    ...
    def save_table_id(self, table_id: str):
        with open(root_path / 'Administrator_Scripts/Nocodb/table_id.txt', 'w', encoding='utf-8') as f:
            f.write(table_id)
    ...
    def load_table_id_from_file(self, table_name: str):
        try:
            with open(root_path / 'Administrator_Scripts/Nocodb/table_id.txt', 'r', encoding='utf-8') as f:
    ...
    def add_new_row(self, table_name: str, new_row: dict):
        # http://localhost:8080/api/v1/db/data/nc/p8akf3nfpn5q59z/mt3n4yq5uqm4tjo
        response = requests.post(
