...
NC_ADMIN_EMAIL = os.getenv('NC_ADMIN_EMAIL', 'arman@gmail.com')
...
logger.info(
    f"Topic 3-Add_Database - Consumer Connection... - Port: {KAFKA_HOST_PORT}")
...
logger.info('Topic 3-Add_Database - Connection Successful')
...
def connect_to_database():
    ...
    if URI is not None:
        ...
        DATABASE_SCHEMA = os.getenv('DATABASE_SCHEMA', 'arman')
        ...
        DATABASE_USER = os.getenv('DATABASE_USER', 'arman')
        DATABASE_PASSWORD = os.getenv('DATABASE_PASSWORD', 'arman')
        DATABASE_PORT = os.getenv('DATABASE_PORT', 16584)
...
def add_to_database(data):
    for _ in range(5):
        try:
            cursor.execute(f'''
                INSERT INTO users  AS u (username, email, id, first_name, last_name, gender, address, post_code, dob, registered_date, phone, picture, company)
                VALUES ('{data['username']}', '{data['email']}', '{data['id']}', '{data['first_name']}', '{data['last_name']}', '{data['gender']}', '{data['address']}', '{data['post_code']}', '{data['dob']}', '{data['registered_date']}', '{data['phone']}', '{data['picture']}', '{data['company']}');
            ''')
            connection.commit()
            break
            ...
            logger.error(
                f"Bad query ! - insert new user section - values ('{data['username']}', '{data['email']}', '{data['id']}', '{data['first_name']}', '{data['last_name']}', '{data['gender']}', '{data['address']}', '{data['post_code']}', '{data['dob']}', '{data['registered_date']}', '{data['phone']}', '{data['picture']}', '{data['company']}')")
            connect_to_database()
            return
        except Exception as e:
            logger.error(f'{e}')
